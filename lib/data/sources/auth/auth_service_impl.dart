...
class AuthServiceImpl implements AuthService {
  ...
  Future<Either> signIn() {
    // TODO: implement signIn -> priority
  ...
  Future<Either> signUp(CreateUser createUser) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return Right('USER CREATED SUCCESSFULLY_______________________ ${response.body}');
      } else {
        // if the user was not created successfully
        // we will return a left with the error message
        return Left('ERROR CREATING USER:_____________________________ ${response.body}');
      }
    } catch (e) {
